    ...
    if ($hasChildren) {
        $checkChildActive = function ($children) use (&$checkChildActive) {
            foreach ($children as $child) {
                if (isset($child['path']) && request()->is($child['path'])) {
                    return true;
                }
                if (isset($child['url']) && request()->is($child['url'])) {
                    return true;
                }
                if (!empty($child['children']) && $checkChildActive($child['children'])) {
                    return true;
                }
            }
            return false;
...
@if ($level === 0)
    ...
    <div class="nav-item-wrapper">
        @if ($hasChildren)
            <a class="nav-link dropdown-indicator label-1" href="#{{ $item['id'] }}" role="button"
                ...
                <div class="d-flex align-items-center">
                    ...
                    @if (isset($item['indicator']))
                        <span class="{{ $item['indicator']['class'] }}"
                            style="{{ $item['indicator']['style'] ?? '' }}"></span>
                    @endif
                    @if (isset($item['badge']))
            ...
            <div class="parent-wrapper label-1">
                <ul class="nav collapse parent {{ $shouldExpand ? 'show' : '' }}"
                    data-bs-parent="{{ $parentBsId ?? '#navbarVerticalCollapse' }}" id="{{ $item['id'] }}">
        ...
        @else
            <a class="nav-link label-1 {{ $isActive ? 'active' : '' }}" href="{{ url($item['url']) }}"
                @if (isset($item['target'])) target="{{ $item['target'] }}" @endif role="button"
...
@else
    ...
    <li class="nav-item">
        @if ($hasChildren)
            <a class="nav-link dropdown-indicator" href="#{{ $item['id'] }}" data-bs-toggle="collapse"
                aria-expanded="{{ $shouldExpand ? 'true' : 'false' }}" aria-controls="{{ $item['id'] }}">
                <div class="d-flex align-items-center">
                    <div class="dropdown-indicator-icon-wrapper">
                        <span class="fas fa-caret-right dropdown-indicator-icon"></span>
                    </div>
            ...
            <div class="parent-wrapper">
                <ul class="nav collapse parent {{ $shouldExpand ? 'show' : '' }}" data-bs-parent="{{ $parentBsId }}"
                    id="{{ $item['id'] }}">
                    @foreach ($item['children'] as $child)
                        @include('layouts.partials.sidebar._menu_recursive', [
                            'item' => $child,
        ...
        @else
            <a class="nav-link {{ $isActive ? 'active' : '' }}" href="{{ url($item['url']) }}"
