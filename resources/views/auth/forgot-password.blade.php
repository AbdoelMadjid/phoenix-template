...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" dir="ltr" data-navigation-type="default" data-navbar-horizontal-shape="default">
  ...
  <head>
    ...
    <!--    Document Title-->
    <!-- ===============================================-->
    <title>{{ config('app.name', 'Phoenix') }} - Forgot Password</title>
              ...
              <div class="text-center">
                ...
                <form method="POST" action="{{ route('password.email') }}" class="mb-5">
                  ...
                  <div class="d-flex align-items-center">
                    <input class="form-control flex-1 @error('email') is-invalid @enderror" id="email" type="email" name="email" value="{{ old('email') }}" required autofocus placeholder="Email">
                    <button type="submit" class="btn btn-primary ms-2">Send<span class="fas fa-chevron-right ms-2"></span></button>
                  </div>
                  @error('email')
                    <div class="text-danger fs-9 mt-1 text-start">{{ $message }}</div>
                  @enderror
                </form>

                @if (Route::has('login'))
                  <a class="fs-9 fw-bold" href="{{ route('login') }}">Back to Sign In</a>
                @endif
              </div>
            </div>
          </div>
